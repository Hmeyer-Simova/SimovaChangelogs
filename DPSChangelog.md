...
Dataset sequence doesn't work | | Huschke Meyer
Added Html encoding to prevent cross-site-scripting | | Huschke Meyer
Wrong file name when downloading a report | | Lukas Böhm
Card page columns aligment (left/right) not possible | | Christian Lienau
Error Messages (Toast message) are empty | | Christian Lienau
Input field configuration |  | Huschke Meyer  
Removed unmask password symbol | | Huschke Meyer
Dashboard Group activation does not work | | Huschke Meyer
Datepicker in basket editing does not work | | Huschke Meyer
Missing Form information| | Huschke Meyer
Use standard SMTP setup | | Markus Czernia
Cleanup Web Users | | Huschke Meyer
Action caption visual glitch | | Huschke Meyer
Update MaterializeCSS to Latest Stable | | Huschke Meyer
Update Jquery to Latest Stable | | Huschke Meyer
Entries cannot be edited from the shopping cart | | Christian Lienau
DPS - extend CSS insert to requests | | Christian Lienau
Added Changelog.md | | Huschke Meyer
